  ...
  return <main className="draw-admin-page">
    <section className="draw-admin-card">
      ...
      <form onSubmit={submit} className="draw-admin-form">
        ...
        <label><span>Copa Intercontinental destino</span><select name="destinationCompetitionId" required><option value="">Seleccionar</option>{competitions.filter((competition) => competition.name.toLocaleLowerCase("es").includes("intercontinental")).map((competition) => <option key={competition.id} value={competition.id}>{competition.name} · {competition.seasonName}</option>)}</select></label>
